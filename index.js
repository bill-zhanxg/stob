...
axios.get('https://raw.githubusercontent.com/Discord-AntiScam/scam-links/main/urls.json')
...
client.on('interactionCreate', async interaction => {
    if (interaction.isButton()) {
        ...
        */
        if (interaction.customId == `mod`) {
            let mod = new MessageEmbed()
                ...
                .addFields(
                    ...
                    { name: `**Delete message one by one instead of bulk delete (Support messages older than 14 days & Slow)**`, value: `\`${Prefix}forcedelete <1 - 100>\`` },
        ...
        if (interaction.customId == 'slashcommands') {
            let fun = new MessageEmbed()
                .setAuthor({ name: "Stob Slash Commands:", iconURL: interaction.client.user.displayAvatarURL({ dynamic: true }) })
                .setColor("BLUE")
                .setDescription("If you can not see the slash commands registered in your server, please reinvite the bot use [this link](https://discord.com/api/oauth2/authorize?client_id=882068020153966663&permissions=8&scope=bot%20applications.commands) (No need to kick then invite, just use the link)")
                .addFields(
                    { name: '**Send an embed (Use `\\n` to line break)**', value: '`/embed`' },
                )

            interaction.reply({ ephemeral: true, embeds: [fun] });
        }
        if (interaction.customId == 'bot') {
            interaction.reply(`Here is the bot invite link:\nhttps://discord.com/api/oauth2/authorize?client_id=882068020153966663&permissions=8&scope=bot%20applications.commands`);
        }
        ...
        if (interaction.customId == 'loop') {
            ...
            await interaction.reply(m == null ? 'Changed!' : { embeds: [new MessageEmbed().setTitle(`Error: ${m}`).setColor("RED")], ephemeral: true });
            if (m == null) try { interaction.deleteReply(); } catch { };
        }
        ...
        if (interaction.customId == 'leave') {
            ...
            await interaction.reply(m == null ? 'Left!' : { embeds: [new MessageEmbed().setTitle(`Error: ${m}`).setColor("RED")], ephemeral: true });
            if (m == null) try { interaction.deleteReply(); } catch { };
        }
    ...
    if (interaction.isContextMenu()) {
        if (interaction.commandName === 'Explain Code') {
            logCommands(2, interaction.user?.tag, interaction.user.displayAvatarURL({ dynamic: true }), interaction?.commandName, interaction.guild?.name, interaction.channel?.id);
    ...
    if (interaction.isSelectMenu()) {
        ...
        if (interaction.customId == "jumpNoD") {
            ...
            if (!queue) {
                return interaction.reply({ embeds: [new MessageEmbed().setTitle(`❌ There is nothing playing!`).setColor("RED")], ephemeral: true });
            }
            let index = parseInt(interaction.values[0]);
            client.distube.jump(message, index).then(async () => {
        ...
        if (interaction.customId == "activity") {
            ...
            if (!vc.permissionsFor(message.guild.me).has(Permissions.FLAGS.CREATE_INSTANT_INVITE)) return interaction.reply(`I do not have permission to create instand invite in ${vc}!`);
            let invite = await functions.activity(client, vc.id, type)
            if (invite == null) {
                interaction.reply('There is an error while creating invite links!')
            }
            else {
                message.channel.send({ embeds: [new MessageEmbed().setColor('GREEN').setTitle('Started an activity!').setDescription(`Channel: ${vc}\nClick [here](${invite}) to join **${message.components[0].components[0].options.filter(m => m.value == type)[0].label}**`).setFooter({ text: `Started by: ${interaction.member.user.tag}`, iconURL: interaction.member.displayAvatarURL({ dynamic: true }) })] });
                message.delete();
            }
        }
    }
    if (interaction.isCommand()) {
        ...
        if (!command) return;
        logCommands(1, interaction.user?.tag, interaction.user.displayAvatarURL({ dynamic: true }), interaction?.commandName, interaction.guild?.name, interaction.channel?.id);
        try {
            await command.execute(interaction);
        } catch (error) {
            console.error(error);
            interaction.reply({ content: 'There was an error while executing this command!', ephemeral: true }).catch(() => interaction.editReply({ content: 'There was an error while executing this command!', ephemeral: true }).catch(() => { }));
        }
    }
});
...
client.on("messageCreate", async message => {
    ...
    if (message.mentions.members.first() && message.mentions.members.first().id == client.user.id) {
        if (message.channel.permissionsFor(message.guild.me).has(Permissions.FLAGS.SEND_MESSAGES)) {
            message.reply(`My prefix in this server is \`${Prefix}\`, use \`${Prefix}h\` to see how to use me`).catch(() => { });
        }
    }
    ...
    if (disabledCommands.indexOf(command.name) != -1) return message.reply("I'm sorry but this command got disabled by the server owner!");
    ...
    logCommands(0, message.author.tag, message.member.displayAvatarURL({ dynamic: true }), cmd, message.guild.name, message.channel.id);

    if (command.music) message.reply({embeds: [new MessageEmbed().setColor('BLUE').setTitle('Music Command for Stob deprecated!').setDescription('Stob music feature got moved to **Stob YouTube** recently. Invite **Stob YouTube** [here](https://discord.com/api/oauth2/authorize?client_id=978893903346434069&permissions=3164160&scope=bot%20applications.commands)')]});
...
function checkScam(message) {
    if (phishingLinks == null) return sendLog(
            ...
            if (phishingLinks.some(v => v == domain)) {
                ...
                if (levenshtein(domain, 'discord.gift') < 7) {
                    ...
                    let scam = new MessageEmbed()
                        .setTitle('SCAM DETECTION')
                        .setColor('RED')
                        .setDescription('I detect the link is very similar to https://discord.gift/ but it\'s not, please be warned: it\'s probably a phishing link!')
...
function logCommands(type, user, userIconURL, commandName, guild, channel) {
    ...
    if (type === 2) {
        ...
        sendLog(
            `User: ${user}, used command: ${type === 0 ? '-' : '/'}${commandName}, in guild: ${guild}, in channel(id): ${channel}. At ${getAusDateNow()}`,
            new MessageEmbed()
                .setAuthor({ name: `Command Used by: ${user}`, iconURL: userIconURL })
