...
module.exports = {
    ...
    run: async (client, message, args) => {
        if (args[0] == null) return message.reply("Usage: -search <video>")
        client.distube.search(args.join(" "), { limit: 1 }).then(async (song) => {
            ...
            let filter = i => i.message.id == msg.id;
            ...
            collector.on('collect', async i => {
                if (i.customId === 'dlVideoWaudio') {
                    ...
                    const dlCT = message.channel.createMessageComponentCollector({ time: 180000, ft, max: 1 });
                    ...
                    dlCT.on('collect', async col => {
                        ...
                        if (vURL == null) {
                            ...
                            col.reply({ ephemeral: true, embeds: [erroEmbed] });
                            return;
                        }
                        ...
                        col.reply({ ephemeral: true, embeds: [btEmbed] }).catch(console.error);
                    })
                }
                    ...
                    dlCT.on('collect', async col => {
                        ...
                        col.reply({ ephemeral: true, embeds: [btEmbed] });
                    })
                }
