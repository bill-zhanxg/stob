const { MessageEmbed, Permissions, Client, BaseGuild, Guild } = require('discord.js');
...
let x = (module.exports = {
	serverInvite: (message) => {
		if (!message.channel.permissionsFor(message.guild.me).has(Permissions.FLAGS.CREATE_INSTANT_INVITE))
			return message.reply("I don't have permission to create invite in this channel!");
		message.channel
			.createInvite({
				maxAge: 1800, //30 mins
				maxUses: 10,
			...
			.catch((error) => {
				message.channel.send(`There is an error while creating the invite link: ${error}`);
			});
	},
	spam: async (message, args) => {
		return message.reply(
			'This command has been disabled because it got abused byy people and I got IP banned by Discord',
		);
		let id = message.guild.id;
		if (cooldown.indexOf(id) != -1)
			return message.reply('Please wait till this command finish processing to use this command again!');
	...
	activity: async (client, channel, type) => {
		return client.together
			.generateTogetherCode(channel, type)
			.then(async (invite) => {
				return invite.code;
			})
			.catch(() => {
		...
		if (longWords)
			for (let longWord of longWords) {
				...
				longWord.split('').forEach((c) => {
					if (
						shortWords[count1] &&
						(shortWords[count1].length >= maxMessageLength * (19 / 20) ||
							(c == '\n' && shortWords[count1].length >= maxMessageLength * (19 / 20) - 100))
					)
						count1++;
	...
	sendLogs: async (client) => {
		...
		function checkWebhokk(guild) {
			const value = cache.get(guild.id);
			...
			return guild
				...
				.then((webhooks) => {
					...
					const channel = re.first().channel;
					...
					if (log) {
						send(
							...
							new MessageEmbed()
								.setColor('RED')
								.setAuthor({
									name: `Channel delete: ${channel.name}`,
									iconURL: channel.guild.iconURL({ dynamic: true }),
								})
								.setThumbnail(log.executor.displayAvatarURL({ dynamic: true }))
								.addFields(
						...
						if (log) {
							send(
								...
								new MessageEmbed()
									.setColor('BLUE')
									.setAuthor({
										name: `Channel Updated: ${newChannel.name} by ${log.executor.tag}`,
						...
						if (log) {
							send(
								...
								new MessageEmbed()
									.setColor('BLUE')
									.setAuthor({
										name: `Emoji Updated: ${newEmoji.name}`,
										iconURL: newEmoji.guild.iconURL({ dynamic: true }),
									})
					...
					if (log) {
						send(
							...
							new MessageEmbed()
								.setColor('RED')
								.setAuthor({
									name: `Banned Member: ${ban.user.tag}`,
					...
					if (log.action == 'MEMBER_BAN_REMOVE') {
						send(
							...
							new MessageEmbed()
								.setColor('GREEN')
								.setAuthor({
									name: `Unbanned Member: ${ban.user.tag}`,
									iconURL: ban.user.displayAvatarURL({ dynamic: true }),
								})
								.setThumbnail(ban.guild.iconURL({ dynamic: true }))
								.addFields(
						...
						send(
							...
							new MessageEmbed()
								.setColor('GREEN')
								.setAuthor({
									name: `Member Ban removed: ${ban.user.tag}`,
									iconURL: ban.user.displayAvatarURL({ dynamic: true }),
								})
								.setThumbnail(ban.guild.iconURL({ dynamic: true }))
								.addFields(
					...
					if (member.user.bot == true) {
						...
						send(
							...
							new MessageEmbed()
								.setColor('GREEN')
								.setAuthor({
									name: `Bot Joined: ${member.user.tag}`,
					...
					if (log.action == 'MEMBER_KICK') {
						send(
							...
							new MessageEmbed()
								.setColor('RED')
								.setAuthor({
									name: `Member Kicked: ${member.user.tag}`,
							...
							if (newMember.communicationDisabledUntilTimestamp == null) {
								send(
									...
									new MessageEmbed()
										.setColor('GREEN')
										.setAuthor({
											name: `Member Untimeouted: ${newMember.user.tag} by ${log.executor.tag}`,
								...
								send(
									...
									new MessageEmbed()
										.setColor('RED')
										.setAuthor({
											name: `Member Timeouted: ${newMember.user.tag} by ${log.executor.tag}`,
											iconURL: newMember.guild.iconURL({ dynamic: true }),
										})
										.setThumbnail(newMember.displayAvatarURL({ dynamic: true }))
										.setDescription(`<@${newMember.id}>`)
										.addField(
											'Remaining timeout',
											`${
												x.secondsToDhms((newMember.communicationDisabledUntilTimestamp - Date.now()) / 1000) == ''
													? '0 Second'
													: x.secondsToDhms((newMember.communicationDisabledUntilTimestamp - Date.now()) / 1000)
											}`,
										),
							...
							if (text != null) {
								send(
									...
									new MessageEmbed()
										.setColor('BLUE')
										.setAuthor({
											name: `Member Updated: ${newMember.user.tag} by ${log.executor.tag}`,
											iconURL: newMember.guild.iconURL({ dynamic: true }),
										})
										.setThumbnail(newMember.displayAvatarURL({ dynamic: true }))
										.setDescription(`<@${newMember.id}>\n${text}`),
						...
						if (log) {
							send(
								...
								new MessageEmbed()
									.setColor('BLUE')
									.setAuthor({
										name: `Guild Updated: ${newGuild.name} by ${log.executor.tag}`,
				...
				if (hook != null) {
					send(
						...
						new MessageEmbed()
							.setColor('BLUE')
							.setAuthor({
								name: `Invite Created: **${invite.code}**`,
					...
					if (log) {
						send(
							...
							new MessageEmbed()
								...
								.setThumbnail(log.executor.displayAvatarURL({ dynamic: true }))
								.addFields({ name: `Invite Deleted by:`, value: `${log.executor.tag}` }),
						);
					}
				}
			})
					...
					sendArray(hook, [
						new MessageEmbed()
							...
							.addFields(
								...
								{
									name: `Content Embeds:`,
									value: `${message.embeds?.length < 1 ? 'No' : '**Yes, Here is the embeds:**'}`,
								},
							),
						...message.embeds,
					]);
					...
					if (log) {
						send(
							...
							new MessageEmbed()
								.setColor('RED')
								.setAuthor({
									name: `Message Bulk Deleted in: ${message.channel?.name}`,
									iconURL: message.guild.iconURL({ dynamic: true }),
								})
								.setDescription(`<#${message.channel?.id}>`)
								.setThumbnail(message.author.displayAvatarURL({ dynamic: true }))
								.addFields(
									...
									{
										name: `First Message Content:`,
										value: `${message.content != '' ? message.content : 'No Message Content'}`,
									},
								),
						);
					}
				...
				if (hook != null) {
					...
					send(
						...
						new MessageEmbed()
							.setColor('BLUE')
							.setAuthor({
								name: `Message Edited in: ${newMessage.channel?.name}`,
						...
						if (text != null) {
							send(
								...
								new MessageEmbed()
									.setColor('BLUE')
									.setAuthor({
										name: `Role Updated: ${newRole.name} by ${log.executor.tag}`,
					...
					if (log.actionType == 'CREATE') {
						send(
							...
							new MessageEmbed()
								.setColor('GREEN')
								.setAuthor({
									name: `Webhook Created in: ${channel.name}`,
									iconURL: channel.guild.iconURL({ dynamic: true }),
								})
								.setDescription(`<#${channel.id}>`)
								.setThumbnail(log.executor.displayAvatarURL({ dynamic: true }))
						...
						send(
							...
							new MessageEmbed()
								.setColor('RED')
								.setAuthor({
									name: `Webhook Deleted in: ${channel.name}`,
									iconURL: channel.guild.iconURL({ dynamic: true }),
								})
								.setDescription(`<#${channel.id}>`)
								.setThumbnail(log.executor.displayAvatarURL({ dynamic: true }))
						...
						send(
							...
							new MessageEmbed()
								.setColor('BLUE')
								.setAuthor({
									name: `Webhook Edited in: ${channel.name}`,
									iconURL: channel.guild.iconURL({ dynamic: true }),
								})
								.setDescription(`<#${channel.id}>`)
								.setThumbnail(log.executor.displayAvatarURL({ dynamic: true }))
		...
		function compareChannel(oldChannel, newChannel) {
			...
			if (nsfw != newChannel.nsfw)
				returnText += `**NSFW state Changed:**\n*Before*: ${nsfw == true ? 'Yes' : 'No'}\n*After*: ${
					newChannel.nsfw == true ? 'Yes' : 'No'
				}\n`;
			if (parent != newChannel.parent)
				returnText += `**Category Changed:**\n*Before*: ${parent.name}\n*After*: ${newChannel.parent.name}\n`;
		...
		function compareRole(oldRole, newRole) {
			...
			if (mentionable != newRole.mentionable)
				returnText += `**Mentionable:**\n*Before*: ${mentionable == true ? 'Yes' : 'No'}\n*After*: ${
					newRole.mentionable == true ? 'Yes' : 'No'
				}\n`;
		...
		function compareGuildMember(oldMember, newMember) {
			...
			if (nickname != newMember.nickname)
				returnText += `**Nickname Changed:**\n*Before*: ${nickname == null ? 'None' : nickname}\n*After*: ${
					newMember.nickname == null ? 'None' : newMember.nickname
				}\n`;
			if (oldAvt != newAvt)
				returnText += `**Server Avatar Changed:**\n*Before*:\n[[here](${oldAvt})]\n*After*:\n[[here](${newAvt})]\n`;
		...
		function compareGuild(oldGuild, newGuild) {
			...
			if (name != newGuild.name)
				returnText += `**Server Name Changed:**\n*Before*: ${name}\n*After*: ${newGuild.name}\n`;
			if (oldIcon != newIcon)
				returnText += `**Server Icon Changed:**\n*Before*:\n[[here](${
					oldIcon == null ? 'None' : oldIcon
				})]\n*After*:\n[[here](${newIcon == null ? 'None' : newIcon})]\n`;
			if (oldBanner != newBanner)
				returnText += `**Server Banner Changed:*Before*:\n[[here](${
					oldBanner == null ? 'None' : oldBanner
				})]\n*After*:\n[[here](${newBanner == null ? 'None' : newBanner})]\n`;
			if (defaultMessageNotifications != newGuild.defaultMessageNotifications)
				returnText += `**Default Message Notifications Changed:**\n*Before*: ${
					defaultMessageNotifications == 'ALL_MESSAGES' ? 'All Messages' : 'Only Mentions'
				}\n*After*: ${newGuild.defaultMessageNotifications == 'ALL_MESSAGES' ? 'All Messages' : 'Only Mentions'}\n`;
			if (systemChannel != newGuild.systemChannel)
				returnText += `**Server System Channel Changed:**\n*Before*: ${
					systemChannel == null ? 'None' : systemChannel
				}\n*After*: ${newGuild.systemChannel == null ? 'None' : newGuild.systemChannel}\n`;
			if (afkChannel != newGuild.afkChannel)
				returnText += `**Server Inactive Channel Channel Changed:**\n*Before*: ${
					afkChannel == null ? 'None' : afkChannel.name
				}\n*After*: ${newGuild.afkChannel == null ? 'None' : newGuild.afkChannel.name}\n`;
			if (afkTimeout != newGuild.afkTimeout)
				returnText += `**Server Inactive Timeout Changed:**\n*Before*: ${afkTimeout} seconds\n*After*: ${newGuild.afkTimeout} seconds\n`;
			if (verificationLevel != newGuild.verificationLevel)
				returnText += `**Verification Level Changed:**\n*Before*: ${verificationLevel}\n*After*: ${newGuild.verificationLevel}\n`;
			if (nsfwLevel != newGuild.nsfwLevel)
				returnText += `**NSFW Level Changed:**\n*Before*: ${nsfwLevel}\n*After*: ${newGuild.nsfwLevel}\n`;
			if (mfaLevel != newGuild.mfaLevel)
				returnText += `**MFA Level Changed:**\n*Before*: ${mfaLevel == 'NONE' ? 'None' : 'Elevated'}\n*After*: ${
					newGuild.mfaLevel == 'NONE' ? 'None' : 'Elevated'
				}\n`;
		...
		function comparePermission(oldPermissionOverwrites, newPermissionOverwrites) {
			...
			returnText += `${addNewPermText != '' ? `**Added new Permission Overwrites**\n${addNewPermText}` : ''}${
				removePermText != '' ? `**Removed Permission Overwrites for**\n${removePermText}` : ''
			}${editPermText != '' ? `**Edited Permission**\n${editPermText}` : ''}`;
